...
class DocValidationController extends Controller
{
    ...
    public function destroy(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($ids) {
            ...
            DocValidationDetails::whereIn('doc_validation_id', $ids)->delete();

            // Then delete parent(s)
            \App\Models\DocValidation::whereIn('id', $ids)->delete();
        });

        return redirect()->back()->with('delete', 'Selected Doc Validations Deleted');
    }


    public function delete_doc_details(Request $request)
    {
        DB::table('doc_validation_details')
            ->where('id', $request->id)
            ->delete();
