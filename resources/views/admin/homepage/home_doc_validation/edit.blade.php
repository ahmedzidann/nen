...
@section('titleadmin') Homepage - Edit Doc Validation @endsection
...
<div class="page-wrapper"><div class="page-content">
    ...
    <form method="POST" action="{{ route('admin.homepage.home-doc-validation.update', $record->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                        ...
                        <div class="row g-3 mt-1">
                            <div class="col-md-12">
                                ...
                                <input type="text" name="title[{{ $item->key }}]"
                                    value="{{ $record->translate('title', $item->key) }}"
                                    class="form-control" placeholder="Section title">
                            </div>
                            <div class="col-md-12">
                                <label class="form-label fw-semibold">Description ({{ ucfirst($item->name) }})</label>
                                <textarea name="description[{{ $item->key }}]" class="form-control" rows="3">{{ $record->translate('description', $item->key) }}</textarea>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Button Text ({{ ucfirst($item->name) }})</label>
                                <input type="text" name="button_text[{{ $item->key }}]"
                                    value="{{ $record->translate('button_text', $item->key) }}"
                                    class="form-control" placeholder="e.g. See More">
                            </div>
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="row g-3 mt-1">
                    <div class="col-md-8">
                        ...
                        <input type="url" name="button_url" value="{{ $record->button_url }}" class="form-control" placeholder="https://...">
                    </div>
                    <div class="col-md-4">
        ...
        <div class="card mb-3">
            ...
            <div class="card-body" id="items-container">
                @foreach($record->items as $idx => $item)
                    @include('admin.homepage.home_doc_validation.partials.item-template', ['item' => $item, 'idx' => $idx, 'translation' => $translation, 'translationFirst' => $translationFirst])
                @endforeach
            </div>
        ...
        <div class="d-flex gap-2">
            <button type="submit" class="btn btn-primary px-4"><i class="bx bx-save me-1"></i>Update</button>
...
@section('jsadmin')
<script>
let itemIndex = {{ $record->items->count() }};
...
document.addEventListener('click', function (e) {
    if (e.target.closest('.remove-item-btn')) {
        ...
        if (itemId) {
            fetch('{{ route("admin.homepage.home-doc-validation.delete-item") }}', {
                method: 'POST',
