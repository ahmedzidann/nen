...
class t extends HTMLElement {
    ...
    constructor() {
        ...
        if (e) {
            ...
            (a.textContent = e), t.append(a);
        }
        const { props: a } = this.constructor;
        if (a)
            for (const [t, e] of Object.entries(a))
                e.attribute &&
                    ((this.#t[t] = e.attribute), (this.#e[e.attribute] = t)),
                    e.type === Boolean && this.#a.add(t);
    }
...
const e = function (t, e, i, r) {
        ...
        if (("string" == typeof t && (t = r.querySelector(t)), !t)) return;
        const s = r.createElementNS("http://www.w3.org/2000/svg", "svg");
        return (
            s.setAttribute("version", "1.1"),
            e && s.setAttribute("width", String(e)),
            i && s.setAttribute("height", String(i)),
            e && i && s.setAttribute("viewBox", "0 0 " + e + " " + i),
            t.appendChild(s),
            (n.svg = s),
            n
        );
    ...
    n = {
        ...
        easeInQuart: function (t, e, a, i) {
            return a * (t /= i) * t * t * t + e;
        },
        easeOutQuart: function (t, e, a, i) {
            return (t /= i), -a * (--t * t * t * t - 1) + e;
        },
        ...
        easeInQuint: function (t, e, a, i) {
            return a * (t /= i) * t * t * t * t + e;
        },
        easeOutQuint: function (t, e, a, i) {
            return (t /= i), a * (--t * t * t * t * t + 1) + e;
        },
...
class u extends t {
    ...
    static get observedAttributes() {
        return Object.entries(this.props).map(
            ([t, e]) => (e && "object" == typeof e && e.attribute) || t
        );
    }
    static defaults = {
        startAngle: 0,
    ...
    constructor(t = {}) {
        ...
        super(),
            ...
            (a =
                "valueOnCircle" === (t = { ...u.defaults, ...t }).textFormat
                    ? 16
                    : 8),
            (this.graph = { paper: e(this.shadowRoot, 100, 100), value: 0 }),
            ...
            this._initText(),
            Object.keys(u.props).forEach((e) => e in t && this._set(e, t[e]));
    }
    attributeUpdated(t, e) {
        this._set(t, e);
    }
    ...
    attr(t) {
        ...
        if ("string" == typeof t) {
            if (1 === arguments.length) return this._get(t);
            t = [[t, arguments[1]]];
        }
        return (
            Array.isArray(t) || (t = Object.keys(t).map((e) => [e, t[e]])),
            t.forEach(([t, e]) => this._set(t, e)),
            this
        );
    }
    _get(t) {
        return this._flushBatch(), this.#r[t];
    }
    _set(t, e) {
    ...
    _flushBatch() {
        ...
        const t = this.#n;
        this.#n = null;
        let e = t.min ?? this.#r.min,
            a = t.max ?? this.#r.max;
        "min" in t && t.min >= a && (e = t.min = a),
            "max" in t && t.max <= e && (a = t.max = e),
            "value" in t &&
                !(t.unconstrained ?? this.#r.unconstrained) &&
                (null != e && t.value < e && (t.value = e),
                null != a && t.value > a && (t.value = a));
        for (const [e, a] of Object.entries(t))
            if (this.#r[e] !== a) {
                if (
                    ...
                    e in h &&
                        (void 0 !== a
                            ? this.graph.paper.svg.setAttribute(h[e], a)
                            : this.graph.paper.svg.removeAttribute(h[e])),
                    ["min", "max", "unconstrained"].includes(e) &&
                    ...
                    this._initText();
                    const t = "valueOnCircle" === a ? 16 : 8;
                    this.graph.sector.attr("stroke-width", t),
                        this.graph.circle.attr("stroke-width", t);
                }
                this.reflectPropToAttribute(e);
            }
        this.updateGraph();
    }
    _formatValue(t, e) {
        switch (t) {
            ...
            case "min":
            case "max":
                (e = Number(e)), Number.isFinite(e) || (e = void 0);
                break;
            case "startAngle":
                (e = Number(e)),
                    (e = Number.isFinite(e)
    ...
    _isIndeterminate() {
        return ["value", "max", "min"].some((t) => "number" != typeof this[t]);
    }
    _positionValueText(t, e) {
        const a = s(e, t);
        this.graph.textVal.attr({ x: 50 + a.x, y: 50 + a.y });
    }
    _initText() {
        ...
        switch (
            ...
            "string" == typeof t &&
                ...
                ["horizontal", "vertical"].includes(t) &&
            ...
            case "valueOnCircle":
                this.graph.textVal.attr({ x: 0, y: 0, dy: "0.4em" }),
                    this.graph.textMax.attr({ x: 50, y: 50, dy: "0.4em" });
                break;
            case "horizontal":
        ...
        this.graph.text.attr("dy", "vertical" === t ? "" : "0.4em");
    }
    #s = null;
    updateGraph() {
        const t = this.startAngle - 90,
            e = this.getRadius();
        if ((this.#s?.cancel(), this._isIndeterminate()))
            this._updateText(this.value, t, e);
        else {
            const a = !this.anticlockwise;
            let i = this._valueToAngle();
            this.graph.circle.attr("r", e),
                "none" !== this.animation && this.value !== this.graph.value
                    ? (this.#s = (function (t, e, a, i, r) {
                          const s = "string" == typeof t ? n[t] : t;
                          let o, h;
                          const u = function (t) {
                              o || (o = t), (t -= o), (t = Math.min(t, i));
                              const n = s(t, e, a, i);
                              r(n),
                                  t < i
                    ...
                    : (this.graph.sector.attr("d", o(50, 50, e, t, i, a)),
                      this._updateText(this.value, (2 * t + i) / 2, e)),
                (this.graph.value = this.value);
        }
    }
    _updateText(t, e, a) {
        if ("function" != typeof this.textFormat) {
            switch (this.textFormat) {
                case "value":
                    const formattedValue = Number.isInteger(t)
                        ? t.toFixed(0)
                        : t.toFixed(1);
                    this.graph.text.el.textContent =
                        void 0 !== t ? formattedValue : this.indeterminateText;
                    break;
                case "percent":
                    const percentValue =
                    ...
                    this.graph.text.el.textContent = formattedPercent + "%";
                    break;
                case "none":
                    this.graph.text.el.textContent = "";
                    break;
                default:
                    const formattedVal = Number.isInteger(t)
                        ? t.toFixed(0)
                        : t.toFixed(1);
                    this.graph.textVal.el.textContent =
                        void 0 !== t ? formattedVal : this.indeterminateText;
                    const formattedMax = Number.isInteger(this.max)
                        ? this.max.toFixed(0)
                        : this.max.toFixed(1);
