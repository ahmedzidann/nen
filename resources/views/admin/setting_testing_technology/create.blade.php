    ...
    <div class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">
                <a href="{{ route('admin.setting_technology_testing.index', $type) }}">
                    Settings
                </a>
            </div>
            <div class="ps-3">
            ...
            <div class="col">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="myForm"
                            action="{{ route('admin.setting_technology_testing.store', $type) }}"
                            method="post"
...
$(document).ready(function() {
    $('#category_id').on('change', function() {
        ...
        // استخدم placeholder "__ID__" داخل route Blade
        let url = "{{ route('admin.get-child-pages', ['page_id' => '__ID__']) }}";
        url = url.replace('__ID__', selectedValue); // استبدل placeholder بالقيمة الفعلية

        $.ajax({
            url: url,
            type: 'GET', // GET أو POST حسب endpoint
            success: function(response) {
                // فرغ select قبل الإضافة
                ...
                if (response.data && response.data.length > 0) {
                    response.data.forEach(function(item) {
                        // أنشئ option لكل child page
                        var option = $('<option></option>')
                            .attr('value', item.slug) // أو id حسب حاجتك
                            .text(item.slug); // أو item.name حسب الاستجابة
