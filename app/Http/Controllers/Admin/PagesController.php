...
class PagesController extends Controller
{
    ...
    public function show(Request $request, $language)
    {
        if ($request->ajax()) {
            ...
            if (!empty($request->sub_parent_id)) {
                $query->where('parent_id', $request->sub_parent_id);
            ...
            return Datatables::of($query)
                ...
                ->addColumn('category', function ($row) use ($language) {
                    return   optional($row->parent)->translate('name', $language);
                })
                ->editColumn('created_at', function ($row) {
                    return \Carbon\Carbon::parse($row->created_at)->format('Y-m-d');
                })
                ->addColumn('action', function ($row) {
                    return '<div class="d-flex order-actions"> <a href="' . route('admin.pages.edit', $row->id) . '" class="m-auto"><i class="bx bxs-edit"></i></a></div>';
                })
    ...
    {
        foreach (Page::find($request->id) as $admin) {
            $admin->delete();
        }
