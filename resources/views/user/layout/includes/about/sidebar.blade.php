        ...
        @if (isset($VCpages))
            @foreach ($VCpages as $page)
                <a href="{{ route('about.' . $page->slug . '') }}"
                    ...
                    <div class="img_link">
                        <img class="@if ($page->slug == 'identity') Identity_icon @endif"
                            src="{{ asset($page->getFirstMediaUrl('icon')) }}" alt="" />{{ $page->name }}
                    </div>
                </a>
            @endforeach
        @elseif (isset($Spages))
            @foreach ($Spages as $page)
                <a href="{{ route('education.' . $page->slug . '', ['page_id' => $page->id]) }}"
                    ...
                    <div class="img_link">
                        <img class="@if ($loop->first) Identity_icon @endif"
                            src="{{ asset($page->getFirstMediaUrl('icon')) }}" alt="" />{{ $page->name }}
                    </div>
                </a>
            @endforeach
            ...
            @foreach ($projectPages as $page)
                <div class="flex_asid_menu">
                    <a class="ref_styles dropdown_arrow {{ Route::is('projects.' . $page->slug . '') ? 'active_link active' : '' }}"
                        href="#">
                        <div class="img_link">
                            <img class="Identity_icon" src="content/images/small_icon/card.png"
                                alt="">{{ $page->name }}
                        </div>
                    ...
                    <ul class="aside_menu">
                        @foreach (\App\Models\Project::where('pages_id', $page->id)->get() as $solution)
                            <li><a class="{{ Route::is('projects.' . $page->slug . '') && $solution->id == Request()->project_id ? 'li_active' : '' }}"
                ...
                <div class="flex_asid_menu">
                    ...
                    <ul class="aside_menu">
                        @foreach (\App\Models\Solution::where('pages_id', $page->id)->get() as $solution)
                            <li><a class="{{ Route::is('solutions.' . $page->slug . '') && $solution->id == Request()->solution_id ? 'li_active' : '' }}"
...
@else
    <div class="aside_div">
                <a href="{{ route('contact-us', ['contact_offices' => 'regional-offices']) }}"
                    class="ref_styles active_ref">
                    <div class="img_link">
                        <img class="Identity_icon" src="" alt="" />Regional offices
                    </div>
                </a>
                <a href="{{ route('contact-us', ['contact_offices' => 'authorized-offices']) }}"
                    class="ref_styles active_ref ">
                    <div class="img_link">
                        <img class=" Identity_icon"
                            src="" alt="" />Authorized Offices
                    </div>
                </a>
                <a href="{{route('contact-us', ['contact_offices' => 'regional-representatives'])}}"
                    class="ref_styles active_ref ">
                    <div class="img_link">
                        <img class=" Identity_icon"
                            src="" alt="" />Regional Representatives
                    </div>
                </a>
    </div>
@endif
